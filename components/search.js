 ...
 * [MAJOR] True Global Deep Search — searches ALL fields, ALL content, ALL entity data.
 *
 * Architecture:
 *   - Single DB call: queryEntities({}) fetches all entities at once (no per-type loops)
 *   - Indexes every string-like value on EVERY entity property, not just declared config fields
 *   - Searches: title, all text/richtext fields (stripped), tags arrays, checklist item text,
 *               multiselect arrays, status, notes, number fields, dates, email, phone, url
 *   - TF-style scoring: title match > field-value match, recency, type priority, overdue boost
 *   - Result snippets: shows which field matched and highlights the matching fragment
 *   - Debounce 80ms, renders in one synchronous pass after async DB fetch
 *
 * Public API:
 ...
 *   addFacet(f)    — pre-filter the search from views
...
/** Cached all-entities snapshot; refreshed on ENTITY_SAVED / ENTITY_DELETED */
...
// ── Type config map (populated at init) ─────────────────────── //
let _typeMap = new Map(); // typeKey → config
...
export function initSearch() {
  ...
  // Build type map once
  for (const cfg of getAllEntityTypes()) {
    _typeMap.set(cfg.key, cfg);
  }
  ...
  // Input handler — commands immediate, search debounced 80ms
  ...
  // Backspace removes last facet when input empty
  ...
  // Invalidate cache on entity changes — next search rebuilds from DB
...
// ════════════════════════════════════════════════════════════════
// ENTITY CACHE — single DB load, invalidated by events
// ════════════════════════════════════════════════════════════════

async function _getEntities() {
  if (!_cacheStale && _entityCache) return _entityCache;
  try {
...
// ════════════════════════════════════════════════════════════════
// DEEP SEARCH — build index from EVERY property on every entity
// ════════════════════════════════════════════════════════════════
...
/**
 * Build a normalised searchable text blob for an entity.
 * Returns { title, titleLower, blob, fieldSnippets }
 * blob = all content concatenated in lower-case
 * fieldSnippets = [{fieldLabel, value}] for matched display
 */
function _buildIndex(entity) {
  const cfg    = _typeMap.get(entity.type);
  ...
  const parts = []; // { label, text } — for snippet building

  // ── Iterate ALL enumerable properties ────────────────────
  const SKIP = new Set(['id','type','createdAt','updatedAt','createdBy','deleted',
                        '_authorName','_authorColor','_authorPersonId']);
  ...
  for (const [k, v] of Object.entries(entity)) {
    ...
    if (typeof v === 'string' && v.length > 0) {
      const clean = fieldCfg?.type === 'richtext' ? _stripHtml(v) : v;
      if (clean.trim()) parts.push({ label, text: clean });

    } else if (typeof v === 'number') {
      parts.push({ label, text: String(v) });

    } else if (Array.isArray(v)) {
      // Tags, multiselect, or checklist
      if (v.length === 0) continue;
      if (typeof v[0] === 'string') {
        // tags / multiselect
        parts.push({ label, text: v.join(' ') });
      } else if (typeof v[0] === 'object' && 'text' in v[0]) {
        // checklist items: [{id, text, done}]
        const itemText = v.map(i => i.text || '').filter(Boolean).join(' ');
...
/**
 * Find the best matching snippet for display.
 * Returns a string like "Details: …matched context…"
 */
...
function _score(entity, cfg, titleLower, lq, titleMatch) {
  ...
  // Type priority
  s += ({ task: 14, event: 10, note: 9, project: 7, person: 6 }[cfg?.key] || 0);
...
async function _renderSearchResults(query) {
  ...
  const matches = []; // { entity, cfg, score, snippet }
  ...
  if (ranked.length === 0) {
    _results.innerHTML = `<div style="padding:var(--space-6) var(--space-5);color:var(--color-text-muted);font-size:var(--text-sm);text-align:center;">No results for "<strong>${_esc(query)}</strong>"</div>`;
    return;
  }
  ...
  for (const { entity, cfg, title, snippet } of ranked) {
    const item = _makeResultItem({
      icon:       cfg.icon || '📎',
      title,
      detail:     snippet || cfg.label || cfg.key,
      color:      cfg.color,
      onActivate: () => {
        closeSearch();
...
async function _renderRecents() {
  ...
  if (recents.length === 0) {
    _results.innerHTML = `<div style="padding:var(--space-4) var(--space-5);color:var(--color-text-muted);font-size:var(--text-sm);text-align:center;">Start typing to search, or type <kbd>&gt;</kbd> for commands</div>`;
    return;
  }
...
const COMMANDS = [
  ...
  { label: 'Settings',      detail: 'Open Settings',            icon: '⚙️', keys: ['settings','preferences'],
  ...
  { label: 'New Person',    detail: 'Open new person form',     icon: '👤',  keys: ['new person','person','add member'],
  ...
  { label: 'Dark Mode',     detail: 'Toggle dark / light theme',icon: '🌙',  keys: ['dark','light','theme'],
    action: () => {
      closeSearch();
...
function _renderCommands(query) {
  ...
  if (cmdService) {
    ...
    const lq = query.toLowerCase();
    matches = (query.length === 0 ? COMMANDS : COMMANDS.filter(c =>
      c.label.toLowerCase().includes(lq) || c.keys.some(k => k.includes(lq))
...
function _handleInputKey(e) {
  switch (e.key) {
    ...
    case 'Enter':
      e.preventDefault();
      if (_selectedIndex >= 0 && _currentItems[_selectedIndex]) _currentItems[_selectedIndex].activate();
      else if (_currentItems.length > 0) _currentItems[0].activate();
      break;
...
function _moveSelection(delta) {
  ...
  if (count === 0) return;
  if (_selectedIndex >= 0 && _currentItems[_selectedIndex]) _currentItems[_selectedIndex].deselect();
...
function _makeSection(label) {
  ...
  if (_results.children.length === 0) { h.style.borderTop = 'none'; h.style.marginTop = '0'; }
  h.textContent = label;
  return { header: h };
}
...
async function _trackRecent(entityId) {
  ...
  try {
    ...
    const cfg   = _typeMap.get(entity.type);
    const title = cfg?.fields?.find(f => f.isTitle)
      ? (entity[cfg.fields.find(f => f.isTitle).key] || 'Untitled')
