/**
 * FamilyHub v4.4.5 — views/daily.js [minor v16]
...
async function _syncDailyReviewLinks(dateStr, data) {
  try {
    ...
    for (const ev of data.events.filter(ev => {
      ...
      const end   = _isoToLocalDate(ev.endDate || ev.date);
      if (!start) return false;
      return start <= dateStr && dateStr <= (end || start);
    })) {
      await _linkToDailyReview(dr.id, ev.id, 'event', linkedSet);
      linkedSet.add(ev.id);
    }
...
function _buildCaptureBar(container, dateStr, sectionRefs) {
  ...
  const doSave = async () => {
    ...
    try {
      ...
      const now     = new Date().toISOString(); // [minor] BUG-63 fix: use toISOString() — sv-SE locale trick is fragile
...
(function _scheduleMidnightRefresh() {
  ...
  setTimeout(() => {
    // [minor] BUG-46/71 fix: only re-render if daily view is active AND user is logged in
    const viewEl = document.getElementById('view-daily');
    if (viewEl && viewEl.classList.contains('active')) {
      renderDaily({ _internal: true });
    }
