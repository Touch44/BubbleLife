...
import { registerView, back as routerBack, navigate } from '../core/router.js';
...
(function _injectStyles() {
  ...
  s.textContent = `
    ...
    .gv-close-btn:hover { opacity: 0.85; }
...
function _buildToolbar(container, activeTypeSet) {
  ...
  div4.className = 'gv-divider';
  toolbar.appendChild(div4);

  // ── Close Graph button — always visible, lets user leave graph view
  const closeBtn = document.createElement('button');
  closeBtn.className = 'gv-close-btn';
  closeBtn.title = 'Close Knowledge Graph and return to previous view';
  closeBtn.innerHTML = '✕ Close Graph';
  closeBtn.addEventListener('click', () => {
    // [minor] BUG-31/67 fix: do NOT call destroyGraph() here — VIEW_CHANGED handler
    // will call it when navigate/back fires. Double-destroy caused console errors.
    _mounted = false;
    if (!routerBack()) {
      navigate('dashboard');
    }
