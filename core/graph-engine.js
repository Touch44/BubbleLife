...
const BUILT_IN_ENTITY_TYPES = [
  ...
  // ── 1. task ─────────────────────────────────────────────── //
  {
    ...
    fields: [
      ...
      field('status',        'select',   { label: 'Status',   options: ['Not Started', 'Next Up', 'In Progress', 'Completed'] }),
      ...
      field('checklist',  'checklist',  { label: 'Checklist', isTitle: false, hidden: true }),
      ...
      field('timeTracked','number',   { label: 'Time Tracked (s)', hidden: true }),
      // ── Recurrence template fields [v5.3.1] ──────────────────
      field('rrule',              'text',     { label: 'Recurrence Rule', hidden: true }),
      field('recurrenceEnd',      'select',   { label: 'Series Ends',     options: ['never','count','date'], hidden: true }),
      field('recurrenceCount',    'number',   { label: 'End After (occurrences)', hidden: true }),
      field('recurrenceEndDate',  'date',     { label: 'End Date', hidden: true }),
      field('nextOccurrenceDate', 'date',     { label: 'Next Occurrence', hidden: true }),
      field('occurrenceCount',    'number',   { label: 'Total Generated', hidden: true, defaultValue: 0 }),
      field('currentStreak',      'number',   { label: 'Current Streak', hidden: true, defaultValue: 0 }),
      field('longestStreak',      'number',   { label: 'Best Streak', hidden: true, defaultValue: 0 }),
      field('lastCompletedPeriod','text',     { label: 'Last Completed Period', hidden: true }),
      field('instancePreviewDays','number',   { label: 'Preview Days Ahead', hidden: true, defaultValue: 7 }),
      field('instanceKeepDays',   'number',   { label: 'Keep History Days', hidden: true, defaultValue: 30 }),
      field('pausedUntil',        'date',     { label: 'Paused Until', hidden: true }),
    ],
  ...
  // ── 1b. taskInstance ─────────────────────────────────────── //
  // [v5.3.1] Ghost occurrences generated by the recurrence scheduler.
  // Not user-creatable; graphVisible:false keeps it out of the graph UI.
  {
    key:          'taskInstance',
    label:        'Task Occurrence',
    labelPlural:  'Task Occurrences',
    icon:         '↺',
    color:        '#7C3AED',
    isBuiltIn:    true,
    graphVisible: false,
    ...
    fields: [
      ...
      field('occurrenceIndex','number',   { label: 'Occurrence #', hidden: true }),
      field('isGhost',        'checkbox', { label: 'Auto-generated', hidden: true }),
      field('_noSync',        'checkbox', { label: 'Skip sync', hidden: true }),
  ...
  // ── 3. event ────────────────────────────────────────────── //
  {
    ...
    fields: [
      ...
      field('plannedDuration','select',  { label: 'Duration', options: ['15 min','30 min','45 min','1 hour','1.5 hours','2 hours','3 hours','4 hours','6 hours','8 hours'], helpText: 'Duration of this event — used for time-block overlap checking. Auto-computed from Start/End when both are set.' }),
  ...
  // ── 5. project ──────────────────────────────────────────── //
  {
    ...
    fields: [
      field('name',           'title',    { isTitle: true, label: 'Name' }),
      field('context',        'select',   { label: 'Context', options: ['family','personal','business','all'] }),
      field('status',         'select',   { label: 'Status', options: ['Active', 'On Hold', 'Complete', 'Archived'], defaultValue: 'Active' }),
      field('completionMode', 'select',   { label: 'Task Mode', options: ['Parallel', 'Sequential'], defaultValue: 'Parallel' }),
  ...
  // ── v5.0.0: reminder ──────────────────────────────────── //
  // Zero IDB schema change — uses existing entities store type index
  {
    key:          'reminder',
    label:        'Reminder',
    ...
    fields: [
      ...
      field('context',           'select',   { label: 'Context', options: ['family','personal','business','all'] }),
      field('notes',             'richtext', { label: 'Notes', isTitle: false }),
      field('priority',          'select',   { label: 'Priority', options: ['Urgent','High','Normal','Low'] }),
      field('fireAt',            'datetime', { label: 'Fire At', required: true }),
      field('timezone',          'text',     { label: 'Timezone' }),
      field('rrule',             'text',     { label: 'Recurrence Rule' }),
      field('recurrenceEnd',     'select',   { label: 'Ends', options: ['never','count','date'] }),
      field('recurrenceEndDate', 'date',     { label: 'End Date' }),
      field('recurrenceCount',   'number',   { label: 'End After (fires)' }),
      field('intermittent',      'checkbox', { label: 'Re-alert until dismissed' }),
      field('intermittentInterval','number', { label: 'Re-alert interval (min)' }),
      field('intermittentMax',   'number',   { label: 'Max re-alerts' }),
      field('snoozeMinutes',     'number',   { label: 'Default snooze (min)' }),
      field('targetType',        'text',     { label: 'Target Type (dynamic query only)' }),
      field('conditionMode',     'select',   { label: 'Condition', options: ['none','field_match','query'] }),
      field('conditionJson',     'text',     { label: 'Condition JSON', hidden: true }),
      field('channelInApp',      'checkbox', { label: 'In-app alert' }),
      field('channelToast',      'checkbox', { label: 'Toast notification' }),
      field('channelPush',       'checkbox', { label: 'Push notification' }),
      field('channelAudio',      'checkbox', { label: 'Audio tone' }),
      field('audioTone',         'select',   { label: 'Tone', options: ['alarm','bell','chime','ping','gentle'] }),
      field('status',            'select',   { label: 'Status', options: ['active','snoozed','paused','dismissed','expired'] }),
      field('nextFireAt',        'datetime', { label: 'Next Fire', hidden: true }),
      field('lastFiredAt',       'datetime', { label: 'Last Fired', hidden: true }),
      field('fireCount',         'number',   { label: 'Fire Count', hidden: true }),
      field('snoozeUntil',       'datetime', { label: 'Snoozed Until', hidden: true }),
      field('dismissedAt',       'datetime', { label: 'Dismissed At', hidden: true }),
      field('dismissedBy',       'text',     { label: 'Dismissed By', hidden: true }),
      field('isTemplate',        'checkbox', { label: 'Is Template' }),
      field('autoGenerated',     'checkbox', { label: 'Auto-generated', hidden: true }),
      field('firstTargetId',     'text',     { label: 'Primary Target ID', hidden: true }), // [v5.0.0] L-03: SW routing
      // [v5.2.0] Phase 3: chained reminders
      field('chainedTo',         'text',     { label: 'Chained Reminder ID', hidden: true }),
      field('chainDelayDays',    'number',   { label: 'Chain delay (days)', defaultValue: 0 }),
      field('chainDelayHours',   'number',   { label: 'Chain delay (hours)', defaultValue: 0 }),
      field('chainTitle',        'text',     { label: 'Chained reminder title' }),
    ],
  },

  // ── v5.0.0: reminderLog ──────────────────────────────── //
  // type='reminderLog' in entities store — getEntitiesByType('reminderLog') pattern
  {
    key:          'reminderLog',
    label:        'Reminder Log',
    labelPlural:  'Reminder Logs',
    icon:         '📋',
    color:        '#94a3b8',
    isBuiltIn:    true,
    graphVisible: false,
    ...
    actions:      [],
    fields: [
      field('title',      'title',    { isTitle: true, label: 'Entry' }),
      field('reminderId', 'text',     { label: 'Reminder ID', hidden: true }),
      field('outcome',    'select',   { label: 'Outcome', options: ['fired','snoozed','done','skipped'] }),
      field('note',       'text',     { label: 'Note' }),
      field('firedAt',    'datetime', { label: 'Fired At', hidden: true }),
      field('resolvedAt', 'datetime', { label: 'Resolved At', hidden: true }),
      field('fireCount',  'number',   { label: 'Fire #', hidden: true }),
      field('targetId',   'text',     { label: 'Target ID', hidden: true }),
    ],
  },

  // ── [v5.2.0] Phase 3: Auto-Rule entity type ─────────────── //
  {
    key:          'rule',
    label:        'Rule',
    labelPlural:  'Rules',
    ...
    color:        '#6366f1',
    isBuiltIn:    true,
    hidden:       true,          // not shown in sidebar nav; managed via auto-rules engine
    graphVisible: false,
    fields: [
      field('name',        'text',     { label: 'Rule Name', isTitle: true }),
      field('triggerType', 'select',   { label: 'Trigger', options: [
        { value: 'entity:saved',    label: 'Entity saved' },
        { value: 'entity:created',  label: 'Entity created' },
        { value: 'status:changed',  label: 'Status changed' },
        { value: 'due:overdue',     label: 'Becomes overdue' },
      ]}),
      field('targetType',  'select',   { label: 'Apply To', options: [
        { value: 'task',    label: 'Tasks' },
        { value: 'event',   label: 'Events' },
        { value: 'project', label: 'Projects' },
        { value: 'any',     label: 'Any entity' },
      ]}),
      field('conditionJson', 'text',   { label: 'Conditions (JSON)', hidden: true }),
      field('actionType',  'select',   { label: 'Action', options: [
        { value: 'create:reminder', label: 'Create a reminder' },
        { value: 'set:status',      label: 'Set status' },
        { value: 'notify',          label: 'Send notification' },
      ]}),
      field('actionConfig', 'text',    { label: 'Action config (JSON)', hidden: true }),
      field('enabled',     'checkbox', { label: 'Enabled', defaultValue: true }),
      field('lastFiredAt', 'datetime', { label: 'Last fired', hidden: true }),
      field('fireCount',   'number',   { label: 'Times fired', hidden: true }),
      field('notes',       'text',     { label: 'Notes', multiline: true }),
    ],
    actions: ['archive'],
    views:   ['list'],
    defaultView: 'list',
